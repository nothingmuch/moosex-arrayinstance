...
use ok('MooseX::ArrayInstance');

{
	package Point;
    use metaclass "Moose::Meta::Class" => ( instance_metaclass => "MooseX::ArrayInstance" );
	use Moose;
	...
	sub clear {
	    my $self = shift;
	    $self->meta->find_attribute_by_name("x")->set_value($self, 0);
	    $self->y(0);
	}
...

{
	...
	after 'clear' => sub {
	    my $self = shift;
	    $self->meta->find_attribute_by_name("z")->set_value($self, 0);
	};
...
is($point3d->[-1], 3, '... got the right value for z');
...
is($point3d->[-1], 0, '... got the right (cleared) value for z');
